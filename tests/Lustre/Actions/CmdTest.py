...
class ActionsTest(unittest.TestCase):
    ...
    def test_format_target_mdt_quota_v24x(self):
        """test command line format v2.4.x (MDT with quota)"""
        Globals().replace('lustre_version', '2.4.2')
        self.fs.new_target(self.srv1, 'mgt', 0, '/dev/root')
        tgt = self.fs.new_target(self.srv2, 'mdt', 0, '/dev/root')
        tgt.full_check(mountdata=False)
        action = Format(tgt, quota=True, quota_type='ug')
    ...
    def test_format_target_ost_quota_v20x(self):
        """test command line format v2.0.x (OST with quota)"""
        Globals().replace('lustre_version', '2.0.0.1')
        self.fs.new_target(self.srv1, 'mgt', 0, '/dev/root')
        tgt = self.fs.new_target(self.srv2, 'ost', 0, '/dev/root')
        tgt.full_check(mountdata=False)
    ...
    def test_format_target_ost_quota_v24x(self):
        """test command line format v2.4.x (OST with quota)"""
        Globals().replace('lustre_version', '2.4.2')
        self.fs.new_target(self.srv1, 'mgt', 0, '/dev/root')
        tgt = self.fs.new_target(self.srv2, 'ost', 0, '/dev/root')
        tgt.full_check(mountdata=False)
    ...
    def test_tunefs_target_quota_v24x(self):
        """test command line tunefs quota (v2.4.x)"""
        Globals().replace('lustre_version', '2.4.2')
        self.fs.new_target(self.srv1, 'mgt', 0, '/dev/root')
        mdt = self.fs.new_target(self.srv1, 'mdt', 0, '/dev/root')
        action = Tunefs(mdt, quota=True, quota_type='ug')
