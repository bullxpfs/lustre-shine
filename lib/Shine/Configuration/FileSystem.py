...
from Shine.Configuration.ModelFile import ForbiddenChangeError
...
class FileSystem(object):
    ...
    @classmethod
    def validate_change(cls, elems, actions, obj=None, type=None):
        """Check that changes requested by update are legal."""
        forbiddendict = {}
        ...
        for elem in elems:
            ...
            for key in chgset:
                if key not in old.changemap():
                    forbiddendict.setdefault(key, []).append(str(old))
                else:
